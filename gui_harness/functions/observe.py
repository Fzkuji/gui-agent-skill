...
observe() is an @agentic_function that:
  ...
  4. Returns a structured ObserveResult dict
"""
...
# Shared default runtime (lazy-initialized)
...
@agentic_function
def observe(task: str, app_name: str = None, runtime: GUIRuntime = None) -> dict:
    ...
    Args:
        ...
        runtime:    Optional: GUIRuntime instance (uses default if None).
    ...
    Returns:
        dict with keys:
            ...
            target_visible (bool)
            target_location (dict | None)  — {x, y, w, h, label}
            screenshot_path (str)
    """
    rt = runtime or _get_runtime()

    # 1. Gather screen data
    if not app_name:
    ...
    # Run detection (graceful: falls back to OCR-only if YOLO unavailable)
    try:
...
Analyze the screenshot and the data above. Return JSON:
{{
  "app_name": "{app_name}",
  "page_description": "short description of what page/state this is",
  "visible_text": ["list", "of", "key", "text", "labels"],
  "interactive_elements": ["list", "of", "clickable", "element", "names"],
  "target_visible": true/false,
  "target_location": {{"x": 0, "y": 0, "w": 0, "h": 0, "label": "element"}} or null,
